...
// GET /api/follow-ups
followupsRouter.get('/', requireAnyPermission(['follow_ups:create', 'follow_ups:view']), async (req, res, next) => {
  try {
...
followupsRouter.get('/:id', requireAnyPermission(['follow_ups:create', 'follow_ups:view']), async (req, res, next) => {
  try {
    const authReq = req as AuthRequest;
