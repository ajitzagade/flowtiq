...
- [ ] `apps/mobile/ios/fastlane/Fastfile` with `beta` lane
- [ ] `apps/mobile/ios/fastlane/Appfile` configured
- [ ] `apps/mobile/ios/fastlane/Matchfile` configured
- [ ] `apps/mobile/ios/Gemfile` and `Gemfile.lock` committed
- [ ] App Store Connect API key auth in beta lane
- [ ] iOS deployment target 15.0 in Xcode and Podfile
- [ ] Manual signing for Release config
- [ ] `ios:release` pnpm script added
- [ ] GitHub Actions secrets documented
- [ ] README: setup steps, prerequisites, match init instructions
- [ ] Local beta build succeeds (verified on Mac with Xcode)
